# Client-Request-Adaptation Capability

## Purpose

将不同客户端发送的模型 API 请求转换为 Smart-Provider 内部稳定的请求表示，使 Ingress 层和上游转发层不必关心客户端协议的差异。当前主要支持 OpenAI 兼容协议，未来可通过新增适配器扩展对其他客户端格式的支持。
