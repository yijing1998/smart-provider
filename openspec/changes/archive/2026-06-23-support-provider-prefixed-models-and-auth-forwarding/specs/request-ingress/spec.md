...
#### Scenario: 客户端发送 provider-prefixed 模型名

- **WHEN** 客户端发送 `POST /v1/chat/completions` 且请求体中 `model` 为 `openai/z-ai/glm-5.1`
- **THEN** 请求接入层 SHALL 接受该请求，不返回模型不识别错误
...
- **WHEN** 客户端请求的模型名为无效 provider 前缀（如 `unknown/model`）或不在 litellm 静态表中
