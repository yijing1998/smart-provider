...
| 阶段 | 能力 | 说明 |
|------|------|------|
| 1 | RPM 限速 | ✅ 已完成，基于滑动窗口控制每分钟请求数。 |
| 2 | 流式响应 | ✅ 已实现，支持 OpenAI 兼容 SSE 流式输出。 |
| 3 | TPM 限速 | 在 RPM 基础上引入 Token 计数，覆盖长文本场景。 |
| 4 | 优先级队列 | 为不同客户端或请求类型分配优先级。 |
| 5 | 熔断器 | ✅ 已实现，在上游持续异常时快速失败，避免无效请求堆积。 |
| 6 | 分布式限速 | 多实例间共享限速状态，支持横向扩展。 |
